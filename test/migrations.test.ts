...
describe('SQLite migrations', () => {
  it('upgrades a pre-ledger database and creates a backup before rebuilding constraints', async () => {
    ...
    expect(
      verification
        .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'qa_search'")
        .get(),
    ).toEqual({
      name: 'qa_search',
    });
    ...
    expect(
      verification
        .prepare(
          "SELECT name FROM sqlite_master WHERE type = 'table' AND name LIKE 'guided_execution_%'",
        )
        .all(),
    ).toEqual(
      expect.arrayContaining([
        { name: 'guided_execution_phases' },
        { name: 'guided_execution_tasks' },
        { name: 'guided_execution_decisions' },
      ]),
    );
