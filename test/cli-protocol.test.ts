...
describe('CLI protocol', () => {
  ...
  it('emits a versioned JSON workflow contract without opening workspace storage', async () => {
    ...
    expect(result.data.workflows).toHaveLength(5);
  ...
  it('describes the same workflow contracts used by execution', () => {
    ...
    expect(contracts.find((workflow) => workflow.id === 'plan-build-qa')).toMatchObject({
      ...
      steps: [
        { id: 'scope', profile: 'analyst' },
        { id: 'plan', profile: 'planner' },
        { id: 'build', profile: 'builder' },
        { id: 'qa', profile: 'qa' },
        { id: 'fix', profile: 'builder' },
      ],
    });
    expect(contracts.find((workflow) => workflow.id === 'todo-build-qa')).toMatchObject({
      version: 1,
      requiredProfiles: ['analyst', 'builder', 'qa'],
      input: { required: ['objective', 'todo'] },
    });
